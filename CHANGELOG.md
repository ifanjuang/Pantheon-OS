...
- **Veto Arès — patterns DTU/sécurité chantier** : les patterns regex DTU, infaisabilité, danger structurel et sécurité des personnes migrent de `_HEPHAISTOS` vers `_ARES` dans `veto_patterns.py`. Héphaïstos (diagram_builder) ne porte plus de veto — Arès (security_guard, veto=True) prend en charge toute la sécurité systémique.
- **SOUL.md — réécriture des 8 agents alignés** : Apollon (validation transverse & méta-vérification), Argos (extraction & structuration de données), Héphaïstos (constructeur de diagrammes & livrables techniques), Arès (gardien de sécurité systémique), Déméter (collecte & agrégation de contexte), Hadès (mémoire longue durée & archives profondes), Poséidon (distributeur de charge & routage multi-agents), Aphrodite (styliste de livrables & mise en forme production).
